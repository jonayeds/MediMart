...
const placeOrderValidation = z.object({
    medicines: z.array(medicinesValidationSchema).min(1, { message: "At least one medicine is required" }),  // Array of medicines, must contain at least one
  customer: z.string().min(1, { message: "Customer ID is required" }),  // Ensure customer ID is a valid string
